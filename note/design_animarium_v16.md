...
**v0.16 — 2 settembre 2026**

*v0.16: la scala cambia di due ordini di grandezza — **251 comuni invece di
tredici**, l'Emilia-Romagna al 95% della popolazione piu' Brescia e Mantova
come presidio lombardo (§3.1). L'atlante diventa **coropleta** (§4.8): 330
poligoni comunali dal dissolve delle sezioni, colorati sui marcatori
demografici della popolazione SINTETICA, con i comuni sotto soglia in grigio.
Il messaggio si rovescia — non piu' «undici su 328, l'assenza e'
informazione» ma la copertura come oggetto, e il grigio che continua a dire
il limite. Milano esce dal bundle pubblicato: 27 MB contro il tetto di 25 MB
per file di Cloudflare Pages (§15).*
...
bundle/ comuni.json indice: 251 voci, con baricentro, n_eff e marcatori geo/er_comuni.json 330 poligoni comunali ER, 302 KB (§4.8) medie_nazionali.json comuni/036023/ pop.parquet 43 colonne, 10 row group, 3,47 MB manifest.json etichette, ordini, conteggi non filtrati riferimenti.json conteggi censuari da cs_K9C.json
...
**251 citta': 99,8 MB**, 4.565.997 individui — 20,9 byte per individuo,
coerente con i 19,7 misurati su Modena alla v0.9: il formato scala, non c'e'
sorpresa nella taglia.

I **pacchetti di deposito** (`*.tar.gz` per Zenodo) stanno in `pacchetti/`,
mai in `bundle/`: `deploy.py` copia il bundle da disco e pubblicherebbe 35 MB
che non servono a nessuno. (Il filtro `ignore_patterns("*.tar.gz")` in
`deploy.py` li escludeva gia': cintura e bretelle.)
...
Una mappa dei comuni sotto l'intestazione. Alla v0.15 serviva a **dire cosa
non copriamo**: undici comuni su 328, e l'assenza era informazione quanto la
presenza. Con 251 comuni il messaggio si rovescia — la copertura diventa
l'oggetto, e il limite si legge nel grigio.

**Coropleta, non dischi.** 330 poligoni comunali generati da
`build/geo_comuni.py`: dissolve delle sezioni di censimento per `PRO_COM`,
semplificazione a 150 m, riproiezione in WGS84. 322.177 vertici diventano
15.631 (4,9%) e il file pesa **302 KB** — un decimo di un Parquet medio. La
tolleranza ha una ragione misurata: l'atlante e' largo ~900 px per ~250 km,
quindi un pixel vale ~280 m e il dettaglio sotto i 150 m non e'
rappresentabile.

**Nessuna libreria di mappe.** I poligoni si disegnano con gli stessi
`px`/`py` che i dischi usavano — la proiezione Web Mercator era gia' isolata
in due funzioni pure, e applicarla a un anello di coordinate invece che a un
punto e' lo stesso conto. `Path2D` risolve il clic con `isPointInPath`, senza
scrivere un point-in-polygon.

**Cinque indicatori, in scala di RANGO.** Popolazione, quota di stranieri,
eta' mediana, laureati fra i 25+, banda `n/n_eff`. I tre demografici sono
aggregati dalla popolazione **sintetica** in `build_indice.py`, non letti da
ISTAT: se la geografia sociale che ne esce e' riconoscibile — Bologna 36,9%
di laureati contro Reggio 22,7%, la gerarchia universitaria della regione —
la mappa argomenta da sola. La scala e' per rango perche' quella lineare
schiaccerebbe il 99% dei comuni nel primo 2% (Bologna 390.098 contro Zerba
74): il rango mostra la geografia relativa, il valore assoluto sta nella
scheda al clic.

**Il grigio dichiara il limite.** Gli 85 comuni sotto la soglia di inclusione
(meno di 3.000 abitanti) sono disegnati ma non colorati, e la nota sotto la
mappa dice perche': sotto quella taglia il campione sarebbe piu' piccolo del
supporto del modello. Il criterio di §7 del paper finisce sotto la mappa,
dove chi guarda si chiede cosa sia quel grigio.

**Brescia e Mantova a margine, non escluse.** Stanno fuori dall'inquadratura
regionale e compaiono come pastiglie tratteggiate nell'angolo, con
l'etichetta «fuori regione». Sono le uniche col pool AVQ lombardo, quindi
`n_eff` diverso, ed e' il caso piu' istruttivo che il bundle contenga:
nasconderlo lo appiattirebbe. Sono anche le uniche a conservare i dischi —
poligoni non ne hanno, in questa vista.

**I dischi ER sono spariti.** A 238 comuni erano un groviglio illeggibile, e
sono diventati ridondanti: il colore dice il valore, il poligono il confine,
un contorno spesso il comune corrente. La repulsione e la lineetta alla
posizione vera restano nel codice per il caso senza GeoJSON — il viewer
degrada ai dischi invece di rompersi.

**Il clic apre una scheda** prima di cambiare citta': individui,
articolazione, tier, e la banda con `n/n_eff` piu' una riga che traduce il
numero. La frase e' **condizionale al rapporto**, non cablata: dire «un
comune piccolo non satura il pool» sotto la banda ×10,9 di Bologna era il
contrario della verita'.

> La copertura del constraint set (67/333 per i K9C, 26/96 per i K6C) stava
> nella scheda ed e' stata tolta il 2/9: e' una proprieta' del **disegno dei
> blocchi**, identica per tutti i comuni dello stesso livello, quindi in una
> scheda che serve a confrontarli non discrimina nulla. Resta nel pannello
> dei marginali, dove dice se QUEL filtro ha un confronto censuario.
---
...
#### Due attributi che il bundle NON porta

Dal 5/8/2026 la popolazione ha due derivazioni in più: il **titolo di
studio dettagliato** (`gsp.istruzione`, dal censimento 2011) e la coppia
**settore × posizione professionale** (`gsp.lavoro`).

Nessuna delle due entra nel Parquet, ed è una scelta coerente col regime
pubblico: sono attributi che rendono riconoscibile una **persona**, non
che servono a una mappa o a un marginale. Vivono nei regimi `persona` e
`narrativo`, generati da Python quando servono.

Il che significa anche che il pannello non li può filtrare né mostrare,
e non deve prometterlo.
...
**Provato su 251 comuni senza toccare l'interfaccia** — la prova richiesta
alla v0.9 con undici, rifatta a scala di due ordini di grandezza superiore.
Fra loro due livelli (K9C ×11, K6C ×240), quattro denominazioni di zona,
quattro tier e comuni da 799 a 390.098 abitanti: il pannello si adatta dal
manifest. 252 su 253 in 474 secondi; l'unico fuori e' San Vito dei Normanni,
segnalato come sorgente assente, che non blocca gli altri.

**Un comune si esclude dal bundle con `bundle: false` nel registro**, e
l'esclusione compare nel riepilogo invece di essere subita in silenzio.
Milano e' esclusa dal 2/9: 27 MB contro il tetto di 25 MB per file di
Cloudflare Pages. Resta generata e resta nel report come caso di scala
metropolitana; rientra col passaggio dei dati su R2 (§7), che quel tetto non
ce l'ha.

251 citta' ≈ 4,57 milioni di individui e **99,8 MB** di bundle, di cui ~2,5
scaricati per sessione.
---
...
| coropleta invece dei dischi nell'atlante | 238 comuni leggibili invece di un groviglio |
| scala di rango invece che lineare | Bologna non schiaccia piu' 250 comuni nel primo 2% |
| geometrie dal dissolve delle sezioni | 302 KB, nessuna fonte esterna |
| marcatori demografici in `comuni.json` | la mappa colora la popolazione sintetica, non ISTAT |
| `allow_nan=False` in `build_indice` | il NaN falliva in scrittura invece di rompere il viewer |
| `bundle: false` nel registro | Milano fuori dal pubblicato, dichiarato nel riepilogo |
...
- **avviso nella mappa del comune per i 13 senza coordinate**: la collocazione
  per sezione c'e' (64, 32, 75 sezioni distinte su tre campioni), ma `lon` e'
  nullo — la tela resta vuota senza dire perche'. Causa a monte, da guardare:
  quasi tutti piacentini e parmensi, quindi un pattern nell'ANNCSU o
  nell'aggancio civici-sezioni, non un caso;
- **`deploy.py` scriva un'intestazione «GENERATO — non modificare qui»** in
  cima a `deploy/index.html`: mezza giornata persa il 2/9 modificando la copia
  invece del sorgente (`build/pannello_marginali.html`), e il primo deploy l'ha
  sovrascritta;
- **`serve_range.py` suggerisca `deploy/index.html`** invece dei prototipi in
  `build/`: i suggerimenti stampati all'avvio sono fermi a quando il pannello
...
### 13.7 La scala v2 — 251 comuni (2 settembre 2026)

| misura | valore |
|---|---|
| comuni nel bundle | 251 (240 K6C + 11 K9C) |
| individui | 4.565.997 |
| bundle | 99,8 MB · **20,9 byte/individuo** |
| byte/individuo per fascia | <3k: 31,2 · 3-10k: 24,3 · 10-50k: 21,3 · >50k: 20,0 |
| costruzione | 474 s per 252 comuni |
| geometrie | 330 poligoni, 15.631 vertici, 302 KB |

L'overhead dei file piccoli esiste ma e' modesto (+55% sui microcomuni, 0,2 MB
in tutto): i metadati Parquet non si ammortizzano su 799 individui, ma la
scala regge senza interventi.

**Marcatori, primi valori** (aggregati dalla popolazione sintetica):

| comune | stranieri | eta' mediana | laureati 25+ |
|---|---:|---:|---:|
| Bologna | 15,2% | 47 | 36,9% |
| Brescia | 18,9% | 48 | 26,1% |
| Parma | 17,4% | 47 | 29,3% |
| Modena | 15,4% | 48 | 27,8% |
| Reggio nell'Emilia | 16,8% | 46 | 22,7% |

La gerarchia dei laureati riproduce quella universitaria della regione senza
esserci stata messa a mano: e' il tipo di conferma che una coropleta puo'
dare e una tabella no.

> **L'eta' mediana esce a numeri interi** (46, 47, 48): `eta_anni` e' intera e
> la mediana di una popolazione grande cade quasi sempre su un intero. Per una
> coropleta significa pochi gradini di colore — la media, o meglio l'indice di
> vecchiaia (65+/under-15), sarebbe piu' discriminante. Da valutare.

**13 comuni senza baricentro.** `lon`/`lat` nulli in Rottofreno, Pontenure,
Caorso, Ponte dell'Olio, Sarmato, Vernasca, Busseto, Fornovo di Taro, Soragna,
Polesine Zibello, Savignano sul Rubicone, Novafeltria, Voghiera: la
collocazione per sezione c'e', le coordinate no. `json.dump` scriveva `NaN`,
che non e' JSON valido, e il viewer moriva su `.json()` con un errore che
sembrava tutt'altro — l'indice intero rotto da 13 comuni su 251. Cura doppia:
la chiave non si scrive se il valore non e' finito, e `allow_nan=False` fa
fallire la scrittura invece di produrre un file che rompe il lettore.
...
**E la diversità apparente è cresciuta senza che quella reale cambi.**
Un persona-prompt oggi porta nome, titolo di studio dettagliato, settore
e posizione professionale: un agente sembra molto più individuato di sei
mesi fa. Ma titolo e mestiere sono **derivazioni deterministiche** da
attributi già presenti, non informazione nuova — due agenti con la stessa
firma AVQ e lo stesso profilo demografico restano identici dove conta,
e ora si assomigliano meno in superficie. È un peggioramento del
problema, non un miglioramento: rende più difficile accorgersene.

---

## 15. Pubblicazione

### Il flusso, e dove si modifica cosa (2 settembre 2026)

`build/pannello_marginali.html` **e' il viewer**; `deploy/index.html` ne e'
una copia rinominata da `deploy.py`, insieme a `deploy/bundle/`. Modificare
la copia e' lavoro perso: il primo `deploy.py` la sovrascrive.
```
build/pannello_marginali.html          sorgente — QUI si modifica
        ↓ deploy.py
deploy/index.html + deploy/bundle/     copia di pubblicazione — generata
        ↓ wrangler pages deploy
animarium.pages.dev
```


Per provare la pubblicazione: `serve_range.py --dir deploy`, e la pagina e'
`localhost:8000/index.html`. Per lo sviluppo: `serve_range.py` dalla radice, e
la pagina e' `localhost:8000/deploy/index.html` — ma allora `trovaBundle()`
risolve su `deploy/bundle/`, che e' la copia dell'ultima pubblicazione e non i
dati di lavoro. Le due modalita' non si mescolano.

**Il tetto di Cloudflare Pages e' 25 MB per file.** Milano (27 MB) lo supera
ed e' esclusa con `bundle: false`. Il passaggio dei dati su R2 (§7) toglie il
limite e alleggerisce il deploy di 100 MB: e' la soluzione strutturale, non
urgente finche' il bundle sta sotto i limiti di Pages.
...
`gsp.individui` (§8 di `note/fonti_e_pacchetto_v6.md`). Il Parquet e' il
